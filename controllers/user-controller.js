...
exports.getAllUsers = async(req,res) =>{
    ...
    if(users.length===0)
    return res.status(404).json({success:false,message:"No"})
...
exports.deleteUser = async (req,res)=>{
    ...
    if(!user){
        return res.status(404).json({success:false,message:"User to be deleted is not found"})
    }
...
exports.updateUserById = async (req,res)=>{
    ...
    return res.status(200).json({success:true,data:updateUserData})
}
